...
{	
	...
	{
		// ETriggerEvent::Started: This event is triggered when the input action is first activated. For example, when a button is pressed down.
		// ETriggerEvent::Completed: This event is triggered when the input action is completed. For example, when a button is released.
		// ETriggerEvent::Triggered: This event is triggered continuously while the input action is active. For example, while a button is held down.
		// Jumping
		...
		// Adding the Interact action binding
...
void ADungeonEscapeCharacter::Interact()
{
	...
	// Get the start and end points of the shape trace
	Start = FirstPersonCameraComponent->GetComponentLocation();
	End = Start + (FirstPersonCameraComponent->GetForwardVector() * MaxInteractionDistance);
	
	// Draw a red debug line to visualize the shape trace and extend the duration to 5 seconds
